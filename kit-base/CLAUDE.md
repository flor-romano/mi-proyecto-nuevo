...
1. **Generar el curso con `tools/new-course.mjs`** (ver abajo). No se
   ...
   **⚠️ Este listado se desactualiza.** La fuente de verdad de qué
   ...
   - **JS (15)**: `motor-slides.js`, `narrador.js` (voz + `textOf()`),
     ...
     gate por documento leído, paginado en la diapositiva), y
   ...
   - **Resto**: `fonts/` (7 `.woff2`), `header-boilerplate.html`,
     `index-boilerplate.html`, `spec-motor-slides.md`,
     **`PROMPT-CURSO-NUEVO.md`** (el prompt de arranque, que viaja con
     cada curso generado), los **16 tests** de `tools/tests/` y las
     ...
     5 chequeos `check-*`). El detalle de cada una está en
     `kit-base/README.md`.
