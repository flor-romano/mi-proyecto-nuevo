...
(function (global) {
  ...
  function initHotspots(opts) {
    ...
    /* Devuelve la MISMA forma aunque no haya zonas (kit-base v1.9.52):
       antes devolvía `null`, contra lo que promete el encabezado de
       este archivo ("Devuelve { mostrar(clave), limpiar() }"). Un curso
       que guardaba el resultado para dispararlo desde otro lado (los
       botones de una barra de pasos, el caso documentado arriba) se
       comía un `TypeError` en cuanto el selector no encontraba nada —
       típico al reordenar diapositivas o renombrar una clase. */
    if (!zonas.length) return { mostrar: function () {}, limpiar: function () {} };
