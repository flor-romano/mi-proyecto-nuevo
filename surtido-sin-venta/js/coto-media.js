...
(function (global) {
  ...
  function initBgVideos(opts) {
    ...
    function attempt(v) {
      ...
      if (p && p.catch) p.catch(function (err) {
        ...
        if (opts.bgVideoAutoMudo === false || v.muted) {
          if (tap) { tap.hidden = false; tap.setAttribute('data-modo', 'reproducir'); }
          return;
        }
        v.muted = true;
        var p2 = v.play();
        if (p2 && p2.then) {
          p2.then(function () {
            ...
            if (tap) { tap.hidden = false; tap.setAttribute('data-modo', 'sonido'); }
          }).catch(function () {
            v.muted = muted();
            if (tap) { tap.hidden = !videoUsable(v); tap.setAttribute('data-modo', 'reproducir'); }
          });
        }
      });
    }
    ...
    Array.prototype.forEach.call(videos, function (v) {
      ...
      if (tap) tap.addEventListener('click', function () {
        if (!v.paused && v.muted && tap.getAttribute('data-modo') === 'sonido') {
          v.muted = false;
          v.volume = volumeLevel();
          tap.hidden = true;
          return;
  ...
  function initShotSwap(opts) {
    ...
    Array.prototype.forEach.call(grupos, function (shot) {
      ...
      /* `data-shot-swap-nofade` (kit-base v1.9.61): opt-out del fundido
         para UN grupo, puesto en su `[data-shot-swap]`. Viene de la
         migración de "Surtido sin ventas", y lo que generaliza es una
         regla sobre qué PORCIÓN del arte cambia entre variantes: el
         fundido de v1.9.51 funde la imagen ENTERA, así que cuando las
         variantes comparten casi todo el cuadro y solo cambia un panel
         (ahí: la misma página con otra pastilla resaltada), lo que se
         percibe no es "se actualizó ese panel" sino "la pantalla
         entera se apagó un instante" — incluida la columna que en los
         hechos no cambió. Con variantes que cambian de punta a punta
         el fundido sigue siendo lo correcto, y por eso el default no
         se toca: esto es opt-in.
         Ojo, NO es lo mismo que `sinFade`: ese es el caso del ARRASTRE
         de la barra de pasos (transitorio, decidido por el gesto);
         este es una propiedad fija del grupo, decidida por su diseño. */
      function go(n, silencioso, sinFade) {
        ...
        if (!mismo) {
          i = n;
          if (sinFade || shot.hasAttribute('data-shot-swap-nofade')) img.setAttribute('src', srcs[i]);
          else swapSrc(srcs[i]);
        }
        vistos[i] = true;
        sync();
