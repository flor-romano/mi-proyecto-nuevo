...
/* ============================================================
   irASlide(page, id) — kit-base v1.9.65
   ------------------------------------------------------------
   La ÚNICA forma correcta de mover un test de diapositiva, y existe
   porque el mismo bug apareció en TRES archivos distintos antes de que
   alguien lo mirara de frente: `verify-hitboxes.mjs` (§7.09),
   `scroll-audit.mjs` y `hitbox-click-check.mjs`.

   El error es siempre igual: navegar clickeando el `[data-goto]` del
   índice lateral. En un curso con gate de avance `initIndexJumps()`
   deja `disabled` los ítems no visitados —correcto por diseño— y un
   clic sobre un botón `disabled` NO dispara `click`. La navegación
   falla EN SILENCIO y el test sigue su bucle midiendo una y otra vez
   la última diapositiva alcanzable, reportando "0 fallos" sobre un
   curso que casi no recorrió. Es el peor modo de fallar posible para
   un test: no se rompe, miente en la dirección tranquilizadora.

   Acá se navega por el motor (saltea gates a propósito, mismo criterio
   que `?review=1`) y se VERIFICA que haya llegado. Devuelve el id real
   donde quedó, para que quien llame decida qué hacer si no es el que
   pidió — lo que NO puede pasar es auditar en silencio otra cosa.
   ============================================================ */
export async function irASlide(page, id) {
