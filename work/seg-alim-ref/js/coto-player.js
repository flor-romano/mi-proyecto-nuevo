...
(function (global) {
  ...
  function initPlayer(opts) {
    ...
    function initSoundToggle() {
      ...
      if (range) {
        /* BUG REAL, reporte del cliente: mover el slider "tarda en
           impactar... hay que reiniciar para que se cambie". La razón
           original (comentario viejo, ya no aplica) era esperar a que
           el cambio "entrara en el fragmento siguiente" de la
           narración — pero una diapositiva típica (título + un
           párrafo corto) suele narrarse en UN solo fragmento: no hay
           "fragmento siguiente" hasta la próxima diapositiva, así que
           en la práctica el cambio nunca se sentía.
           Fix: además de `input` (que sigue actualizando valor/
           localStorage/UI en cada tick, sin tocar la narración —
           refrescarla ahí cortaría la frase a cada píxel de arrastre),
           el slider escucha `change` — dispara UNA sola vez, al soltar
           el mouse (o, con teclado, en cada flecha, que no tiene
           "arrastre" así que el efecto ya es inmediato) — y ahí sí
           llama `refreshVolume()`. Sin debounce ni temporizador: el
           propio evento nativo ya da la semántica correcta ("terminé
           de mover, aplicá esto"), mismo criterio que ya usaba
           `toggleMute` (gesto discreto → efecto inmediato). */
    ...
    /* ---- Gracia de hover para popovers (Sonido/Locución/Ayuda/
       ...
       una clase (`openClass`) al entrar y la saca recién `delayMs`
       después de salir — cancelable si el mouse vuelve a entrar antes
       (al botón O al popover, da igual, el listener está en el
       contenedor `.d-audio-ctl`/`.d-fab`, que envuelve a los dos). El
       foco por teclado (`:focus-within` en CSS) sigue sin gracia — no
       hay "hueco que cruzar" con Tab, así que no la necesita. */
    /* La gracia arrancó en 3s (kit-base v1.9.37) y el cliente la pidió
       más corta al usarla: 3 segundos alcanzan para cruzar el hueco
       pero dejan el panel "colgado" un rato largo después de que el
       alumno ya siguió con otra cosa. 1,5s sigue siendo muchísimo para
       un gesto de mouse (cruzar 10-14px lleva ~50ms) y el panel deja
       de estorbar. Un solo lugar para el número: los 4 popovers usan
       esta constante, no un 1500 repetido en cada llamada. */
    ...
    function initPinnedPopover(items, opts) {
      ...
      /* Cierra los DOS mecanismos de apertura, no solo el pin.
         BUG REAL (kit-base v1.9.40, reporte del cliente: "al hacer clic
         afuera del desplegable que se cierre"): esto sacaba solo
         `is-open`, así que un popover abierto por HOVER (`is-hover`) no
         se cerraba al hacer clic afuera — se quedaba hasta que venciera
         la gracia, aunque el alumno ya hubiera dicho "quiero cerrar
         esto" con el clic. Mismo olvido afectaba a `slidechange` y al
         botón de "volver a ver la introducción", que también llaman
         acá. Un popover tiene UNA sola noción de "cerrado": las dos
         clases se sacan juntas siempre. */
      function closeAll() {
        items.forEach(function (it) { it.classList.remove('is-open', 'is-hover'); });
      }
