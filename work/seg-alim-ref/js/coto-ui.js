/* ============================================================
   ...
   Contenido:
     ...
     · countTo(el, to, opts)  cuenta de un valor CUALQUIERA a otro, repetible
...
(function (global) {
  ...
  /* ---- countTo(el, to, opts) — cuenta de un valor a otro (kit-base v1.9.44) ----
     Generalización de `initStatPopups` para el caso "el número YA tiene
     un valor en pantalla y tiene que subir/bajar hasta uno nuevo" (ej.
     el chip de puntos del header en cada `award()`) — `initStatPopups`
     no sirve para esto: arranca siempre desde 0, corre una sola vez por
     elemento (`data-counted`) y está atada al evento `popupopen`, nada
     de lo cual aplica a un contador que cambia repetidas veces durante
     el curso. `opts.from` es opcional — sin él, parte del `textContent`
     actual del elemento (`parseInt`, `0` si no es un número). Una nueva
     llamada mientras la anterior sigue en curso la reemplaza limpio
     (token por elemento) en vez de competir por el mismo `textContent`
     — necesario porque `award()` puede dispararse varias veces seguidas
     (ej. varios hallazgos del minijuego en poco tiempo). */
  function countTo(el, to, opts) {
    ...
    if (from === null || from === undefined) {
      from = parseInt(el.textContent, 10);
  ...
  /* ---- Sonidos de UI (tonos sintetizados, Web Audio API) ------------
   ...
   -------------------------------------------------------------------- */
  var actx;
  function ac() { if (!actx) { try { actx = new (global.AudioContext || global.webkitAudioContext)(); } catch (e) {} } return actx; }
  ...
  function initGlossaryUnlock(opts) {
    ...
    /* BUG REAL, reporte del cliente sobre "Seguridad alimentaria": un
       término BLOQUEADO seguía navegando igual al tocarlo — `.is-locked`
       solo cambiaba estilos (texto gris, candado, definición oculta),
       nunca tocaba el propio `[data-goto]`. El motor cablea CUALQUIER
       `[data-goto]` con un `click` genérico, sin mirar gates (§6.19
       punto 1: es "ir para ATRÁS a lo ya visto", nunca se pensó para
       poder saltar ADELANTE) — así que un término bloqueado se
       convertía en un atajo para saltarse el bloqueo de "andá
       interactuando diapo por diapo", exactamente el mismo agujero que
       ya tenía el índice/sidenav antes de que `marcarVistas()`
       aprendiera a poner `disabled` (CLAUDE.md §6.44 punto 2) — acá
       nunca se copió ese mismo fix al agregar el glosario interactivo.
       Fix: mismo patrón, `link.disabled = bloqueado` en el propio
       `<button data-goto>` — un botón nativo `disabled` no dispara
       `click`, así que el listener genérico del motor ni se entera,
       sin tener que enseñarle al motor sobre "bloqueado". */
