...
(function (global) {
  ...
  /* Nivel del control global "Sonido" (0-1, panel del header — lo
     escribe `initSoundToggle` en coto-player.js). Mismo helper,
     copiado igual, que ya viven en fx.js / coto-ui.js / coto-media.js:
     cada archivo del kit es autocontenido a propósito (se copian de a
     uno a un curso, no hay módulo compartido). Devuelve 1 ante
     cualquier problema — nunca dejar al alumno sin audio por un
  ...
  /* Volumen EFECTIVO de la locución: el nivel del slider, o 0 si
     "Sonido" está muteado. Mismo criterio que `efectivoMudo()` en
     coto-player.js — mutear "Sonido" tiene que callar TODO lo audible
     (§6.44 punto 9), y una locución que sigue hablando con el curso en
     mute se lee como un bug, no como una excepción a propósito.
     El toggle de "Locución" sigue siendo otra cosa y no se pisa: decide
     si el curso NARRA (preferencia de contenido); éste decide a qué
     volumen suena lo que ya se decidió narrar. */
  ...
  function hablarDesde(i) {
    ...
    u.pitch = 1;
    /* El control "Sonido" del header es un volumen GLOBAL, no solo de
       efectos y video (kit-base v1.9.40, pedido explícito del cliente:
       "me gustaría que el sonido también modifique el volumen de la
       locución"). `SpeechSynthesisUtterance` acepta `volume` 0-1, así
       que la locución se suma al mismo control en vez de necesitar una
       segunda barra propia — dos controles para lo mismo se
       desincronizan y obligan al alumno a bajar el volumen dos veces.
       Es la misma dirección que §6.44 punto 9 ya había fijado para el
       mute ("Sonido mutea TODO, sin excepción"): acá se completa para
       el nivel, no solo para el on/off.
       Se lee en CADA fragmento (no una vez por `speak()`) a propósito:
       una locución larga se parte en varios `SpeechSynthesisUtterance`
       (ver `chunkText`), así que mover el slider a mitad de una
       narración se escucha en el fragmento siguiente en vez de recién
  ...
  /* Re-emite el fragmento EN CURSO para que un cambio de volumen se
     escuche ya (kit-base v1.9.40). `SpeechSynthesisUtterance.volume` se
     fija al crear el utterance y no se puede cambiar mientras suena —
     la única forma real de aplicar un volumen nuevo a lo que ya está
     hablando es volver a emitirlo.
     Lo llama `initSoundToggle` (coto-player.js): SIEMPRE al togglear
     mute (gesto discreto, efecto inmediato de una) y, desde kit-base
     v1.9.42, TAMBIÉN al mover el slider — pero ahí con un debounce de
     ~180ms del lado de `coto-player.js`, no en cada evento `input`
     crudo (eso sí cortaría la frase a cada píxel de arrastre; sin
     ningún refresco durante el drag, en cambio, el cambio "tardaba en
     impactar" — reporte real del cliente, CLAUDE.md §6.62). Sin
     narración activa, o si ya terminó, no hace nada — no re-narra una
     diapositiva que el alumno ya escuchó entera. */
