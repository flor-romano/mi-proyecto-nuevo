...
(function (global) {
  ...
  /* ---- Certificado de finalización imprimible/PDF (kit-base v1.9.39) ----
     Mismo mecanismo ya probado de `initSummaryPrint()` (coto-ui.js):
     `window.print()` + una clase en `<body>` que el CSS usa para tapar
     la app y mostrar SOLO el documento — "imprimir a PDF" es la forma
     estándar del navegador de generar un PDF real sin sumar ninguna
     librería. La diferencia real con el resumen es que el certificado
     NO necesita HTML propio de curso: se arma entero en caliente acá,
     así que un curso nuevo solo agrega el botón `#d-cert-print` (junto
     al de "Imprimir resumen", en el mismo panel de cierre) — cero
     markup del certificado en sí que escribir a mano.

     Pedido pendiente desde CLAUDE.md §6.46 (una de las 10 propuestas de
     esa vuelta, nunca implementada: "mostrar el nombre del alumno en
     el certificado").

     `opts.courseName` es opcional (default `document.title`) — pasarlo
     explícito si el `<title>` real del documento no es un nombre
     presentable (sufijos, siglas internas, etc.).

     Seguridad: el nombre viene del LMS (`cmi.core.student_name`) — igual
     que en `initGreeting()` (coto-player.js), nunca se arma con
     innerHTML ni se confía en que venga limpio; todo el DOM se
     construye con `createElement`/`textContent`. */
  function initCertificatePrint(opts) {
    opts = opts || {};
    var btn = document.getElementById(opts.buttonId || 'd-cert-print');
    if (!btn) return;
    var courseName = opts.courseName || document.title;

    function nombreAlumno() {
      var raw = (global.SCORM && global.SCORM.getFullName && global.SCORM.getFullName()) || '';
      if (!raw) return '';
      // SCORM entrega "Apellido, Nombre" — se invierte para que se lea
      // natural en un certificado ("Nombre Apellido").
      var partes = raw.split(',');
      if (partes.length > 1) return (partes[1] || '').trim() + ' ' + (partes[0] || '').trim();
      return raw.trim();
    }

    function armarDocumento() {
      var doc = document.getElementById('d-cert-doc');
      if (!doc) {
        doc = document.createElement('div');
        doc.id = 'd-cert-doc';
        doc.className = 'd-cert-doc';
        document.body.appendChild(doc);
      }
      doc.textContent = '';

      var card = document.createElement('div');
      card.className = 'd-cert-card';

      var kicker = document.createElement('p');
      kicker.className = 'd-cert-kicker';
      kicker.textContent = 'Certificado de finalización';

      var nombre = document.createElement('h1');
      nombre.className = 'd-cert-nombre';
      nombre.textContent = nombreAlumno() || 'Alumno/a';

      var texto = document.createElement('p');
      texto.className = 'd-cert-texto';
      texto.textContent = 'completó el curso';

      var curso = document.createElement('h2');
      curso.className = 'd-cert-curso';
      curso.textContent = courseName;

      var foot = document.createElement('div');
      foot.className = 'd-cert-foot';
      var fecha = document.createElement('span');
      fecha.textContent = new Date().toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' });
      foot.appendChild(fecha);

      var cajaMedalla = document.querySelector('[data-medalla]');
      var nombreMedalla = document.querySelector('[data-medalla-nombre]');
      if (cajaMedalla && nombreMedalla && cajaMedalla.getAttribute('data-nivel') !== 'ninguna') {
        var medalla = document.createElement('span');
        medalla.className = 'd-cert-medalla';
        medalla.textContent = 'Medalla de ' + nombreMedalla.textContent;
        foot.appendChild(medalla);
      }

      [kicker, nombre, texto, curso, foot].forEach(function (el) { card.appendChild(el); });
      doc.appendChild(card);
    }

    function limpiar() { document.body.classList.remove('printing-cert'); }
  ...
  global.initCertificatePrint = initCertificatePrint;
})(window);
