...
(function (global) {
  ...
  function initShotSwap(opts) {
    ...
    Array.prototype.forEach.call(grupos, function (shot) {
      ...
      /* PARCHE LOCAL (curso "Surtido sin ventas", CLAUDE.md §0.1 — no
         se aplica al kit-base/ canónico desde acá, queda documentado en
         README-CURSO.md para relayar): `data-shot-swap-nofade` en el
         `[data-shot-swap]` deshabilita el fundido de v1.9.51 para ESE
         grupo. En "conceptos", las 8 variantes son la MISMA página con
         una sola pastilla resaltada + el panel derecho — el fundido
         completo de imagen a imagen se leía como que la pantalla entera
         "se apaga" un instante (incluida la columna de la izquierda,
         que en los hechos no cambia) en vez de sentirse como que solo
         el contenido de la derecha se actualiza. Swap instantáneo: sin
         diferencia visible en la columna estática, cambio inmediato en
         la dinámica — candidato genérico a subir al kit como atributo
         declarativo, no algo específico de este curso en su diseño. */
